...
def move_to(to_node):
  ...
  if cur["label"] in ('L', 'SL') and to_node in (cur["left"], cur["right"]):
    print("Du kan ikke gÃ¥ videre fra denne lÃ¥snoden uten Ã¥ lÃ¥se opp fÃ¸rst!")
    return False
  elif to_node == None:
...
def gaa_ut():
  if cur["label"] == 'UT':
    print("Du lÃ¸ste labyrinten. Gratulerer!")
    return True
...
def plukk_opp():
  if not er_nokkel():
    print("Det finnes ingen nÃ¸kkel her.")
    return False
...
def laas_opp():
  if not er_laas():
    print("Denne noden er ikke en lÃ¥s.")
    return False
  
  global keys

  if er_superlaas() and keys < 2:
    print("Du kan ikke lÃ¥se opp denne superlÃ¥sen uten minst to nÃ¸kler.")
    return False
  elif keys < 1:
    print("Du kan ikke lÃ¥se opp denne lÃ¥sen uten en nÃ¸kkel.")
    return False
