...
class SudokuGUI:
    ...
    def __init__(self, master, brett):
        ...
        colorMenu.add_command(label='Blue (default)', command=self.colorSelector)
        colorMenu.add_command(label='Red', command=lambda color='Red': self.colorSelector(color))
        colorMenu.add_command(label='Green Camo', command=lambda color='Green Camo': self.colorSelector(color))
    ...
    def colorSelector(self, color='Blue'):
...
class Kvadrat:
    ...
    def check_coordinate(self, row, col): #tar inn koordinat (0-indeksert), x=rad, y=kollonne, sjekker om det er okkupert
    ...
    def write_to_kvadrat(self, row, col, tall, fortsett = 'nei', delete_mode=False):
        if delete_mode: #sletter
            ...
            if self.check_coordinate(row,col) or fortsett=='ja': #tom celle -> skriv inn 
    ...
    def delete_from_grid(self, row, col):
        self.write_to_kvadrat(row, col, EMPTY_CELL, delete_mode=True)

    def print_grid(self): #brukes ikke
        print('\n'*3)
        for row in range(3):
            for col in range(3):
    ...
    def return_number_at_pos(self, row, col):
        self.grid[row][col]

class Game:

    def print_brett(self, brett): 
        #tar inn brett, printer det formatert riktig

        radnummer = (i for i in range(1,10)) #generer nr 1 til 9, brukes for å skrive ut rad nummer til venstre

        print('   ',1,2,3,' ',4,5,6,' ',7,8,9, sep=' ')
        print(' ','+-------'*3 + '+')
        for grid_row in brett:
            for rad in range(3):
                print(next(radnummer),end=' ')
                for kvadrat in grid_row: #går inn i hvert kvadrat
                    print('| ',end='')
                    for col in range(3):
                        tall = kvadrat.grid[rad][col]
                        if tall != EMPTY_CELL:
                            print(tall,end=' ')
                        else:
                            print(' ', end=' ')
    ...
    def write_to_board_with_input(self, brett):
        # funksjon som skaffe input fra bruker og skriver det til brettet

        try:
            col = int(input('Kollonne: '))
            while col not in range(1,10):
                print('Må være et tall mellom 1 og 9')
                col = int(input('Kolonne: '))
            row = int(input('Rad: '))
            while row not in range(1,10):
                print('Må være et tall mellom 1 og 9')
                row = int(input('Rad: '))
            tall = int(input('Tall: '))
            while tall not in range(1,9):
                print('Må være et tall mellom 1 og 9')
                tall = int(input('Tall: '))
        except:
            print('Input må være et heltall mellom 1 og 9')

        grid_row, grid_col, new_row, new_col = self.konverter_koordinat(row,col)
        brett[grid_row][grid_col].write_to_kvadrat(new_row, new_col, tall)
    ...
    def delete_from_board(self, brett, row, col):
        grid_row, grid_col, new_row, new_col = self.konverter_koordinat(row,col)
        brett[grid_row][grid_col].delete_from_grid(new_row, new_col)
                ...
                for kvadrat_index in range(len(brett[grid_row_index])):
                    for row in range(3):
                        for col in range(3):
                            ...
                            if tall == 0:
                                kvadrat.delete_from_grid(row, col)
                            else:
    ...
    def help_print(self):
        print('"save" lagrer fremskrit')
        print('"import" lar deg importere nytt brett, husk å lagre først!')
        print('"exit" avslutter spillet, husk å lagre først!')
        print('"Når du vil fylle inn brettet skriver du radnummer (venstre), kollonnenummer (øverst), tall"' + \
            '\n altså rad,kollonne,tall (uten mellomrom)')
        print('Rad, kollonne og tall må være heltall mellom 1 og 9')
        print('"slett" for å slette celle')
        print('\n\n')

    def game_cycle_command_line(self, brett): #kjører spillet i cmd vinduet
        print('Sudoku-spill')
        print('Radene skal fylles med tallene 1 til 9')
        print('Kollonnene skal fylles med tallene 1 til 9')
        print('og hvert enkelt 3x3 kvadrat skal fylles med tallene 1 til 9')
        print('Lykke til!' + '\n\n\n')
        print('Skriv "help" for å få liste med kommandoer')
        self.help_print()

        while not self.complete_check(brett):
            user_input = input('Skriv inn kommando: ')

            if user_input.lower() == 'save':
                filename_save = input('Hva skal filen hete? ')
                self.save_sudoku_to_file(filename_save, brett)
                break
            elif user_input.lower() == 'import':
                filename_get = input('Hva heter filen som du vil jobbe på? ')
                self.get_sudoku_from_file(filename_get, brett)
            elif user_input.lower() == 'help':
                help_print()
            elif user_input.lower() == 'slett':
                row = int(input('Rad: '))
                col = int(input('Kollonne: '))
                self.delete_from_board(brett, row, col)
                ...
                try:
                    ...
                    tall = int(user_input[~0])
                    self.write_to_board(brett, row, col, tall)
                except ValueError:
                    print('Kunne ikke tolke kommandoen')
                    continue

            print('\n\n')
            self.print_brett(brett)

    def game_logic(self, brett): 
        # brukes ikke
        # tenkt bruk i GUI-en, funksjon skal kun kjøre essensielle deler av logikken i én runde (ikke loop)
        self.complete_check(brett)
