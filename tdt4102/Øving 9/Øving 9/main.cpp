...
const sf::Color flag_color = sf::Color::Cyan;
...
int main() {
    ...
    int height = 20, width = 30, mines = 100;
    cin >> height >> width >> mines;						// fjern kommentar
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            case sf::Event::MouseButtonPressed:
				if (!game->isGameOver()) {
					...
					else if (event.mouseButton.button == sf::Mouse::Right && !game->isTileOpen(row, col)) {
						game->changeFlag(row, col);
					}
				}
				break;
            }
        ...
        for(int row = 0; row < height; ++row) {
            for(int col = 0; col < width; ++col) {
				...
				// draw flags #ad-hoc
				if (game->isTileFlag(row, col) && !game->isTileOpen(row, col)) {
					tile.setFillColor(flag_color);
				}
